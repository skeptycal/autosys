...
class JSON_file(object):
    ...
    def json_comments(self, comments):
        if not comments:
            self.json_data = json.loads(''.join(line for line in data_file if not line.startswith('//')))
    ...
    def json_format(
        ...
        try:
            self.json_data = json.dumps(self.file_name,
                                        sort_keys=sort_keys,
    ...
    def json_minify(self, comments: bool = False) -> bool:
        """
            ...
            return:
                result (bool)       : True if successful else False
            """
        return self.json_format(self, comments = False, sort_keys=False, separators=(',',':'), indent=None)
