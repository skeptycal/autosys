...
def to_tuple(ippacket, flip=False):
    ...
    if type(payload) is TCPPacket and not flip:
        tup = (
            ippacket.get_src_ip(),
            payload.get_src_port(),  # remote
            ippacket.get_dst_ip(),
            payload.get_dst_port(),
        )  # local
        return tup
    elif type(payload) is TCPPacket and flip:
        tup = (
            ippacket.get_dst_ip(),
            payload.get_dst_port(),  # remote
            ippacket.get_src_ip(),
            payload.get_src_port(),
        )  # local
    else:
        tup = None
    return tup
...
class TCPPacket(TransportLayerPacket):
    ...
    def __unicode__(self):
        ...
        return "TCP from %d to %d" % (self._src_port, self._dst_port)
...
class UDPPacket(TransportLayerPacket):
    ...
    def __unicode__(self):
        ...
        return "UDP from %d to %d" % (self._src_port, self._dst_port)
