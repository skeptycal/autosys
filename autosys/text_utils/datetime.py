""" <<< This is a *fork* of the Python 3.8 datetime.py builtin >>>
    ##############################################################################
    Modifications copyright (c) 2020 Michael Treanor
    Licensed following the original python source code.

    Modifications are labeled with the following label:

        # TODO -- MT -- <description>

    Sections that are refactored or ignored are labeled:

        if True: # TODO -- done (MT) - <description>

    The 'if True' construct is a stupid, but quick code folding helper

    The dbprint() function is added as a quick way to get print output
    during testing. It only prints if _debug_ == True

    The full version information installed was

    - Python 3.8.3 (v3.8.3:6f8c8320e9, May 13 2020, 16:29:34)
    - It was compiled for macOS 10.15.4 using [Clang 6.0 (clang-600.0.57)]
    - Any changes to the updates to the official source code after this version may not be represented or available here.

    * Modifications are for use in this package not intended for under consideration for contribution to the official Python source code repository.
...
if True:  # TODO -- done (MT) - minor changes only
    ...
    def _cmp(x, y):
        # TODO -- MT -- test to see if it is much more likely that x != y, in which case, this test would be more efficient ... just a curiosity ...
        return 1 if x > y else -1 if x < y else 0
        # return 0 if x == y else 1 if x > y else -1

    # TODO -- MT -- why does this need to be 1 to 9999???? are we going to use this program in the year 9999??? I don't understand ...
    MINYEAR = 1
    MAXYEAR = 9999
    _MAXORDINAL = 3652059  # date.max.toordinal()

    # TODO -- MT -- original comment:
...
if True:  # TODO -- done (MT) - this bit seems like a mess ...
    # -1 is a placeholder for indexing purposes.
    _DAYS_IN_MONTH = [-1, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]

    _DAYS_BEFORE_MONTH = [-1]  # -1 is a placeholder for indexing purposes.

    # TODO -- MT -- this *seems* to be a more efficient way ...
    #! profiling needed
...
class timedelta:
    ...
    def __new__(
        ...
        if isinstance(days, float):
            ...
            daysecondsfrac, daysecondswhole = _math.modf(dayfrac *
                                                         (24.0 * 3600.0))
    ...
    def _to_microseconds(self):
        return (self._days *
                (24 * 3600) + self._seconds) * 1000000 + self._microseconds
...
timedelta.max = timedelta(days=999999999,
                          hours=23,
                          minutes=59,
                          seconds=59,
                          microseconds=999999)
    ...
    def fromisocalendar(cls, year, week, day):
        ...
        if not 0 < week < 53:
            ...
            if week == 53:
                ...
                if first_weekday == 4 or (first_weekday == 3
                                          and _is_leap(year)):
                    out_of_range = False
...
class time:
    ...
    def __new__(cls,
        ...
        """
        if (isinstance(hour, (bytes, str)) and len(hour) == 6
...
class datetime(date):
    ...
    def _fromtimestamp(cls, t, utc, tz):
        ...
        if tz is None:
            ...
            if trans.days < 0:
                y, m, d, hh, mm, ss = converter(t +
                                                trans // timedelta(0, 1))[:6]
...
# TODO -- MT -- *# - temporarily ditching this code block for testing ...
"""
try:
